...
static struct profileElement steeralgCmnElementDefaultTable[] = {
    ...
    { STEERALG_APPLY_ESTIMATED_AIRTIME_ON_STRING,    "1" },
...
LBD_STATUS steeralg_init(void) {
    ...
    steeralgState.config.applyEstimatedAirTimeOnSteering =
        profileGetOptsInt(mdModuleID_SteerAlg,
                          STEERALG_APPLY_ESTIMATED_AIRTIME_ON_STRING,
                          steeralgCmnElementDefaultTable);
...
static LBD_STATUS steeralgCmnUpdateCandidateProjectedAirtime(
    ...
    for (i = 0; i < maxNumBSS; i++) {
        ...
        // If ApplyEstimatedAirTimeOnSteering is set to be false in config file and the
        // candidate is on the same channel as the serving BSS, do not update the projected air time
        if(steeralgState.config.applyEstimatedAirTimeOnSteering != LBD_TRUE) {
            stadbEntry_bssStatsHandle_t bssHandle = stadbEntry_getServingBSS(entry, NULL);
            const lbd_bssInfo_t *servingBssInfo = stadbEntry_resolveBSSInfo(bssHandle);

            if(servingBssInfo && servingBssInfo->channelId == candidateList[i].channelId) {
                continue;
            }
        }
...
void steeralgMenuStatusHandler(struct cmdContext *context, const char *cmd) {
    ...
    cmdf(context, "Maximum number of steering target allowed: %u\n",
         steeralgState.config.maxSteeringTargetCount);
    cmdf(context, "Apply client's air time when making steering decision: %u\n",
         steeralgState.config.applyEstimatedAirTimeOnSteering);
    if (steeralgState.offloadState.inProgress) {
