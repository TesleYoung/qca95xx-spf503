...
static int ath_rng_kthread(void *data)
{
    ...
    sc->sc_stats.ast_adc_sync_count_m1_m2 = 0;
    sc->sc_stats.ast_adc_sync_count_m3 = 0;
    sc->sc_stats.last_adc_sync_m1_m2 =0;
    ...
    while (!kthread_should_stop()) {
        ...
        if ((2 == mode) && (rssi_const_count > ATH_RSSI_CONST_COUNT_LIMIT)) {
            ...
            OS_MEMSET(rng_buf, 0, ATH_RNG_BUF_SIZE * sizeof(rng_buf[0]));
            sc->sc_stats.ast_adc_sync_count_m1_m2++;
            rssi_const_count  = 0;
            continue;
        } /* mode 2 */
        ...
        if (3 == mode) {
           ...
           if (unlikely(!bytes_read)) {
               ...
               if (adc_rd_fails > 512){
                   ...
                   adc_rd_fails = 0;
                   printk("%s:%d *** BAD ADC READ *** mode=1\n", __func__, __LINE__);
               }
               continue;
           } else
               adc_rd_fails = 0;
