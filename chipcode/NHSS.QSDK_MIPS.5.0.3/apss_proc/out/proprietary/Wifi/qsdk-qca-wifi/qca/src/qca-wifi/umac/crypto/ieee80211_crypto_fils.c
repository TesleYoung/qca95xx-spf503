...
{
    const u8 *address[FILS_AEAD_NUM_ELEM + 1];
    size_t length[FILS_AEAD_NUM_ELEM + 1];
    ...
    if (aes_siv_encrypt(k->wk_filsaad.kek,  k->wk_filsaad.kek_len,
                fils_session, crypt_len, FILS_AEAD_NUM_ELEM, address, length, buf + (fils_session - ie_start)) <0) {
                ...
                struct ieee80211_rx_status *rs)
{
    ...
    if (aes_siv_decrypt(k->wk_filsaad.kek, k->wk_filsaad.kek_len,
                        fils_session, crypt_len, FILS_AEAD_NUM_ELEM, address, length,
