...
-------------------------------------------------------------------------------
  ...
  $Header: //depot/asic/ipq8066/IPQ8064.ILQ.5.0_P/meta/main/latest/common/build/update_common_info.py#8 $
  $DateTime: 2017/03/28 05:56:29 $
...
if FIT_tool:
   ...
   ipq_gcc5_dir = './ipq_gcc5'
   if not os.path.exists(ipq_gcc5_dir):
      os.makedirs(ipq_gcc5_dir,mode = 0755)
   ipq_allconf_dir = './ipq_allconf'
   ...
   for flash_file in mi.get_files(attr='flash_file_debug'):
      shutil.copy(flash_file, ipq_debug_dir)
      shutil.copy(flash_file, ipq_gcc5_dir)
      shutil.copy(flash_file, ipq_allconf_dir)
   for flash_file in mi.get_files(attr='flash_file_p_debug'):
      shutil.copy(flash_file, ipq_debug_dir)
   for flash_file in mi.get_files(attr='flash_file_gcc'):
      shutil.copy(flash_file, ipq_gcc5_dir)
   ...
   os.rename("./ipq_debug/nor-system-partition-v2.bin", "./ipq_debug/nor-system-partition.bin")
   os.rename("./ipq_gcc5/nor-system-partition-v2.bin", "./ipq_gcc5/nor-system-partition.bin")
   os.rename("./ipq_allconf/nor-system-partition-v2.bin", "./ipq_allconf/nor-system-partition.bin")   

   tools_dir = './ipq/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)	

   tools_dir = './ipq_debug/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)
   
   tools_dir = './ipq_gcc5/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)
   
   tools_dir = './ipq_allconf/tools'
   distutils.dir_util.copy_tree( exported_tools ,tools_dir)
   
   lg.log("update_common_info: Generate ipq single image for gcc5")
   ret_nand_p_gcc = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-p-gcc5-single.img', ipq_gcc5_dir],verbose=0)
   print "ret_nand_p_gcc %d." % ret_nand_p_gcc
   ret_nor_p_gcc = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-p-gcc5-single.img', ipq_gcc5_dir],verbose=0)
   print "ret_nor_p_gcc %d." % ret_nor_p_gcc
  
   lg.log("update_common_info: Generate ipq single image for debug")
   ret_nand_P_debug = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-p-debug-single.img', ipq_debug_dir],verbose=0)
   print "ret_nand_P_debug %d." % ret_nand_P_debug
   ret_nor_P_debug = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-p-debug-single.img', ipq_debug_dir],verbose=0)
   print "ret_nor_P_debug %d." % ret_nor_P_debug
   
   # Streamboost3 binary generation
   ret_streamboost_debug = 0
   if os.path.isfile("./ipq_debug/openwrt-ipq806x-streamboost3-squashfs-rootfs.img"):
		os.rename("./ipq_debug/openwrt-ipq806x-streamboost3-squashfs-rootfs.img", "./ipq_debug/openwrt-ipq806x-squashfs-root.img")
		os.rename("./ipq_debug/openwrt-ipq806x-ipq40xx-ubi-root-streamboost3-enabled.img", "./ipq_debug/openwrt-ipq806x-ipq40xx-ubi-root.img")
		ret_streamboost_debug = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-debug-single-streamboost.img', ipq_debug_dir],verbose=0)
		print "ret_streamboost_debug %d." % ret_streamboost_debug
   
   lg.log("update_common_info: Generate ipq single image")    
   ret_nand_single = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-single.img', ipq_dir],verbose=0)
   print "ret_nand_single %d." % ret_nand_single
   ret_nor_single = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','boardconfig','-o', bin_dir +'/'+'nor-ipq806x-single.img', ipq_dir],verbose=0)
   print "ret_nor_single %d." % ret_nor_single 
   ret_norplusnand_single = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','boardconfig','-o', bin_dir +'/'+'norplusnand-ipq806x-single.img', ipq_dir],verbose=0)
   print "ret_norplusnand_single %d." % ret_norplusnand_single
   ret_nand_apps = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','appsboardconfig', '-o',bin_dir +'/'+'ipq806x-nand-apps.img', ipq_dir],verbose=0)
   print "ret_nand_apps %d." % ret_nand_apps
   ret_nor_apps = lg.log_exec(['python',pack_tool,'-t','nor','-B', '-F','appsboardconfig', '-o',bin_dir +'/'+'ipq806x-nor-apps.img', ipq_dir],verbose=0)
   print "ret_nor_apps %d." % ret_nor_apps
   ret_norplusnand_apps = lg.log_exec(['python',pack_tool,'-t','norplusnand','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-norplusnand-apps.img', ipq_dir],verbose=0)
   print "ret_norplusnand_apps %d." % ret_norplusnand_apps
   ret_emmc_single = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','boardconfig', '-o', bin_dir +'/'+'emmc-ipq806x-single.img', ipq_dir],verbose=0)
   print "ret_emmc_single %d." % ret_emmc_single
   ret_norplusemmc_single = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','boardconfig', '-o', bin_dir +'/'+'norplusemmc-ipq806x-single.img', ipq_dir],verbose=0)
   print "ret_norplusemmc_single %d." % ret_norplusemmc_single
   ret_norplusemmc_apps = lg.log_exec(['python',pack_tool,'-t','norplusemmc','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-norplusemmc-apps.img', ipq_dir],verbose=0)
   print "ret_norplusemmc_apps %d." % ret_norplusemmc_apps
   ret_emmc_apps = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','appsboardconfig', '-o', bin_dir +'/'+'ipq806x-emmc-apss.img', ipq_dir],verbose=0)
   print "ret_emmc_apps %d." % ret_emmc_apps

   # Generate all config single images   
   lg.log("update_common_info: Generate ipq single image for all config")
   ret_nand_allconf_single = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-allconfig-single.img', ipq_allconf_dir],verbose=0)
   print "ret_nand_allconf_single %d." % ret_nand_allconf_single
   ret_emmc_allconf_single = lg.log_exec(['python',pack_tool,'-t','emmc','-B', '-F','boardconfig', '-o', bin_dir +'/'+'emmc-ipq806x-allconfig-single.img', ipq_allconf_dir],verbose=0)
   print "ret_emmc_allconf_single %d." % ret_emmc_allconf_single
   ...
   if os.path.isfile("./ipq/openwrt-ipq806x-streamboost3-squashfs-rootfs.img"):
		...
		ret_streamboost = lg.log_exec(['python',pack_tool,'-t','nand','-B', '-F','boardconfig','-o', bin_dir +'/'+'nand-ipq806x-single-streamboost.img', ipq_dir],verbose=0)
		print "ret_streamboost %d." % ret_streamboost
		
#   if ret_streamboost != 0 or ret_emmc_apps !=0 or ret_norplusemmc_apps !=0 or ret_norplusemmc_single !=0 or ret_emmc_single !=0 or ret_norplusnand_apps !=0 or ret_nor_apps!=0 or ret_nand_apps !=0 or ret_norplusnand_single !=0 or ret_nor_P_debug !=0 or ret_nor_single !=0 or ret_nand_P_debug !=0 or ret_nand_single !=0 or ret_nand_p_gcc !=0 or ret_streamboost_debug !=0 or ret_nor_p_gcc !=0:
