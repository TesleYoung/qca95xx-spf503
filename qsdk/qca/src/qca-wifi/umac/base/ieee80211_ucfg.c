...
int ieee80211_ucfg_check_sta_per(struct ieee80211com *ic, struct sta_val *val)
{
    ...
    if(val->ssid[0])
    {
        ...
        if(j == ATF_CFG_NUM_VDEV)
        {
            ...
            {
                cumulative += ic->atfcfg_set.peer_id[i].sta_cfg_value[j+1];
            }
    ...
    if(i == ATF_ACTIVED_MAX_CLIENTS)
    {
        if(staIndex != 0xff)
        {
            ...
            if(val->ssid[0])
            {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Percentage without ssid is deprecated \n");
