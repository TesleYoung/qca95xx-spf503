...
u_int8_t ieee80211_setup_oce_attrs (int f_type, struct ieee80211vap *vap, u_int8_t *frm, struct ieee80211_node *ni)
{
	...
	cap->ctrl_sta_cfon = 0;
	cap->ctrl_11b_ap_present = oce->ap_11b_present;
...
int ieee80211_check_non_oce_ap(void *arg, wlan_scan_entry_t se)
{
	...
	struct ieee80211_channel *chan = ieee80211_scan_entry_channel(se);
	bool oce_cap = false;
	u_int8_t *mboie;
...
int ieee80211_update_11b_ap_presence (struct ieee80211vap *vap)
{
	...
	struct ieee80211vap *nbr_vap, *tvap;
	ieee80211_oce_scan_cbinfo_t cb_info;
	bool prev_value = oce->ap_11b_present;
	int status = EOK;
	...
	oce->ap_11b_present = cb_info.ap_11b_present;

	if (oce->ap_11b_present != prev_value) {
		vap->iv_flags_ext2 |= IEEE80211_FEXT2_MBO;
		vap->iv_fd_update = 1;
	}
...
bool ieee80211_ap_11b_present (struct ieee80211vap *vap)
{
    if (vap->oce && vap->oce->ap_11b_present)
		return true;
	else
		return false;
}
