				  ...
				  struct hif_driver_state_callbacks *cbk)
{
	...
	if (bus_context_size == 0) {
		HIF_ERROR("%s: context size 0 not allowed\n", __func__);
		return NULL;
	}
...
void hif_close(struct hif_opaque_softc *hif_ctx)
{
	...
	if (scn == NULL) {
		HIF_ERROR("%s: hif_opaque_softc is NULL\n", __func__);
		return;
	}
					  ...
					  enum hif_enable_type type)
{
	...
	if (hif_bus_configure(scn)) {
		HIF_ERROR("%s: Target probe failed.\n", __func__);
		hif_disable_bus(scn);
...
void hif_crash_shutdown(struct hif_opaque_softc *hif_ctx)
{
	...
	if (hif_is_load_or_unload_in_progress(scn)) {
		HIF_ERROR("%s: Load/unload is in progress, ignore!\n", __func__);
		return;
	}
	...
	HIF_INFO_MED("%s: RAM dump collecting completed!\n", __func__);
			...
			uint32_t *hif_type, uint32_t *target_type)
{
	...
	default:
		HIF_ERROR("%s: Unsupported device ID!\n", __func__);
		ret = -ENODEV;
		break;
	}
end:
