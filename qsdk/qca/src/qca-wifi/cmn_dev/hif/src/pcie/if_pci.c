...
void hif_pci_device_warm_reset(struct hif_pci_softc *sc)
{
	...
	HIF_INFO_MED("%s: Target Warm Reset\n", __func__);
	...
	val =
		hif_read32_mb(mem +
			     (SOC_CORE_BASE_ADDRESS |
			      PCIE_INTR_CAUSE_ADDRESS));
	HIF_INFO_MED("%s: Host Intr Cause reg 0x%x : value : 0x%x\n", __func__,
	...
	HIF_INFO_MED("%s: Target CPU Intr Cause 0x%x\n", __func__, val);
	...
	qdf_mdelay(100);
	HIF_INFO_MED("%s: Target Warm reset complete\n", __func__);
...
int hif_check_fw_reg(struct hif_opaque_softc *hif_ctx)
{
	...
	HIF_INFO_MED("%s: FW_INDICATOR register is 0x%x\n", __func__, val);
...
int hif_check_soc_status(struct hif_opaque_softc *hif_ctx)
{
	...
	HIF_INFO_MED("%s: Power register is %08x\n", __func__, val);
...
static void __hif_pci_dump_registers(struct hif_softc *scn)
{
	...
	HIF_INFO_MED("%s: Debug: inputsel: %x dbgctrl: %x\n", __func__,
	       hif_read32_mb(mem + GPIO_BASE_ADDRESS +
			    WLAN_DEBUG_INPUT_SEL_OFFSET),
	       hif_read32_mb(mem + GPIO_BASE_ADDRESS +
			    WLAN_DEBUG_CONTROL_OFFSET));

	HIF_INFO_MED("%s: Debug CE\n", __func__);
	...
	for (i = 0; i < sizeof(wrapper_idx) / sizeof(uint32_t); i++) {
		...
		if (wrapper_idx[i] <= 7) {
			...
			HIF_INFO_MED("%s: out: %x\n", __func__, val);
		}
	}

	HIF_INFO_MED("%s: Debug PCIe:\n", __func__);
	...
	for (i = 0; i <= 8; i++) {
		...
		HIF_INFO_MED("%s: amdbg: %x out: %x %x\n", __func__,
		       hif_read32_mb(mem + GPIO_BASE_ADDRESS +
...
int hif_pci_dump_registers(struct hif_softc *hif_ctx)
{
	...
	if (status)
		HIF_ERROR("%s: Dump CE Registers Failed\n", __func__);
...
static void reschedule_tasklet_work_handler(void *arg)
{
	...
	if (scn->hif_init_done == false) {
		HIF_ERROR("%s: wlan driver is unloaded\n", __func__);
		return;
	}
...
void hif_pci_runtime_pm_warn(struct hif_pci_softc *sc, const char *msg)
{
	...
	list_for_each_entry(ctx, &sc->prevent_suspend_list, list) {
		HIF_ERROR("source %s; timeout %d ms\n", ctx->name, ctx->timeout);
	}
...
static void hif_pm_runtime_start(struct hif_pci_softc *sc)
{
	...
	HIF_INFO("%s: Enabling RUNTIME PM, Delay: %d ms\n", __func__,
...
int hif_set_hia(struct hif_softc *scn)
{
	...
	if (i >= HIF_HIA_MAX_POLL_LOOP) {
		HIF_ERROR("%s: hia polling timeout\n", __func__);
		return -EIO;
	}

	if (host_interest_area == 0) {
		HIF_ERROR("%s: host_interest_area = 0\n", __func__);
		return -EIO;
	}
	...
	if (pcie_state_targ_addr == 0) {
		...
		HIF_ERROR("%s: pcie state addr is 0\n", __func__);
		goto done;
	}
	...
	if (pipe_cfg_targ_addr == 0) {
		rv = QDF_STATUS_E_FAILURE;
		HIF_ERROR("%s: pipe cfg addr is 0\n", __func__);
		goto done;
	}
	...
	if (svc_to_pipe_map == 0) {
		rv = QDF_STATUS_E_FAILURE;
		HIF_ERROR("%s: svc_to_pipe map is 0\n", __func__);
		goto done;
	}
	...
	rv = hif_diag_write_mem(hif_hdl,
			...
			target_service_to_ce_map_sz);
	if (rv != QDF_STATUS_SUCCESS) {
		HIF_ERROR("%s: write svc/pipe map (%d)\n", __func__, rv);
		goto done;
	}
	...
	rv = hif_diag_read_access(hif_hdl,
			...
			&pcie_config_flags);
	if (rv != QDF_STATUS_SUCCESS) {
		HIF_ERROR("%s: get pcie config_flags (%d)\n", __func__, rv);
		goto done;
	}
	...
	if (rv != QDF_STATUS_SUCCESS) {
		HIF_ERROR("%s: write pcie config_flags (%d)\n", __func__, rv);
		goto done;
	}
	...
	rv = hif_diag_read_access(hif_hdl, ealloc_targ_addr,
			&ealloc_value);
	if (rv != QDF_STATUS_SUCCESS) {
		HIF_ERROR("%s: get early alloc val (%d)\n", __func__, rv);
		goto done;
	}
	...
	rv = hif_diag_write_access(hif_hdl,
				...
				ealloc_value);
	if (rv != QDF_STATUS_SUCCESS) {
		HIF_ERROR("%s: set early alloc val (%d)\n", __func__, rv);
		goto done;
	}
#endif
	if ((target_type == TARGET_TYPE_AR900B)
...
int hif_pci_bus_configure(struct hif_softc *hif_sc)
{
	...
	HIF_INFO_MED("%s: hif_set_hia done\n", __func__);
		...
		const struct pci_device_id *id)
{
	...
	if (pci_assign_resource(pdev, BAR_NUM)) {
		HIF_ERROR("%s: pci_assign_resource error\n", __func__);
		return -EIO;
	}
#endif
	...
	if (ret) {
		HIF_ERROR("%s: PCI MMIO reservation error\n", __func__);
		ret = -EIO;
		goto err_region;
	}
	...
	if (ret) {
		HIF_ERROR("%s: Cannot enable 64-bit pci DMA\n", __func__);
		goto err_dma;
	}
	ret = pci_set_consistent_dma_mask(pdev, DMA_BIT_MASK(64));
	if (ret) {
		HIF_ERROR("%s: Cannot enable 64-bit DMA\n", __func__);
		goto err_dma;
	}
#else
	ret = pci_set_dma_mask(pdev, DMA_BIT_MASK(32));
	if (ret) {
		HIF_ERROR("%s: Cannot enable 32-bit pci DMA\n", __func__);
		goto err_dma;
	}
...
int hif_pci_probe_tgt_wakeup(struct hif_pci_softc *sc)
{
	...
	while (!hif_targ_is_awake(scn, sc->mem)) {
		if (0 == targ_awake_limit) {
			HIF_ERROR("%s: target awake timeout\n", __func__);
			ret = -EAGAIN;
			goto end;
		}
		qdf_mdelay(1);
...
int hif_configure_msi(struct hif_pci_softc *sc)
{
	...
	if (num_msi_desired < 1) {
		HIF_ERROR("%s: MSI is not configured\n", __func__);
		return -EINVAL;
	}
	...
	if (rv == 0 || rv >= HIF_MAX_TASKLET_NUM) {
		...
		if (ret) {
			HIF_ERROR("%s: request_irq failed\n", __func__);
			goto err_intr;
		}
		...
		sc->num_msi_intrs = 0;
		ret = -EIO;
		HIF_ERROR("%s: do not support MSI, rv = %d\n", __func__, rv);
	}
	ret = pci_enable_msi(sc->pdev);
	if (ret < 0) {
...
static inline int hif_drain_tasklets(struct hif_softc *scn)
{
	...
	while (qdf_atomic_read(&scn->active_tasklet_cnt)) {
		...
		HIF_INFO("%s: Waiting for CE to finish access\n", __func__);
		msleep(10);
	}
	return 0;
}
...
static int hif_bus_suspend_link_up(struct hif_softc *scn)
{
	...
	if (unlikely(enable_irq_wake(pdev->irq))) {
		HIF_ERROR("%s: Fail to enable wake IRQ!\n", __func__);
		return -EINVAL;
	}
...
static int hif_bus_resume_link_up(struct hif_softc *scn)
{
	...
	if (!pdev) {
		HIF_ERROR("%s: pci_dev is null\n", __func__);
		return -EFAULT;
	}

	if (unlikely(disable_irq_wake(pdev->irq))) {
		HIF_ERROR("%s: Fail to disable wake IRQ!\n", __func__);
		return -EFAULT;
	}
...
void hif_target_sync(struct hif_softc *scn)
{
	...
	if (HAS_FW_INDICATOR) {
		int wait_limit = 500;
		int fw_ind = 0;
		HIF_TRACE("%s: Loop checking FW signal\n", __func__);
		while (1) {
...
again:
	...
	HIF_TRACE("%s: hif_enable_pci done\n", __func__);
	...
	if (ret < 0) {
		HIF_ERROR("%s: invalid device id/revision_id\n", __func__);
		goto err_tgtstate;
	}
...
err_tgtstate:
	...
	HIF_ERROR("%s: error, hif_disable_pci done\n", __func__);
		...
		*hif_sc, struct hif_pm_runtime_lock *lock)
{
	...
	HIF_ERROR("%s: in pm_state:%d ret: %d\n", __func__,
		...
		struct hif_pm_runtime_lock *lock, unsigned int delay)
{
	...
	if (hif_is_recovery_in_progress(sc)) {
		HIF_ERROR("%s: LOGP in progress, ignore!\n", __func__);
		return -EINVAL;
	}
