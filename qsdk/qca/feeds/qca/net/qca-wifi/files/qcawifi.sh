...
load_qcawifi() {
    ...
    if [ -n "$nss_wifi_olcfg" ]; then
        ...
        nss_wifi_olcfg="$(</lib/wifi/wifi_nss_olcfg)"
...
enable_qcawifi() {
    ...
    if [ ! -f /lib/wifi/wifi_nss_override ]; then
        if [ -f /lib/wifi/wifi_nss_olcfg ] && [ $(</lib/wifi/wifi_nss_olcfg) != 0 ]; then
...
setup_wps_enhc() {
    ...
    [ $wps_pbc_overwrite_ap_settings_all -ne 0 ] && append wps_enhc_cfg "-:overwrite_ap_settings_all" "$N"

    config_get_bool wps_pbc_overwrite_sta_settings_all qcawifi wps_pbc_overwrite_sta_settings_all 0
    [ $wps_pbc_overwrite_sta_settings_all -ne 0 ] && append wps_enhc_cfg "-:overwrite_sta_settings_all" "$N"

    config_get wps_pbc_overwrite_ssid_suffix qcawifi wps_pbc_overwrite_ssid_suffix
    [ -n "$wps_pbc_overwrite_ssid_suffix" ] && append wps_enhc_cfg "-:overwrite_ssid_suffix:$wps_pbc_overwrite_ssid_suffix" "$N"
...
pre_qcawifi() {
    ...
    case "${action}" in
        disable)
            ...
            rm -rf /var/run/wifi-wps-enhc-extn.conf
            ...
            rm -rf /var/run/iface_mgr.pid /var/run/iface_mgr.conf
...
post_qcawifi() {
    ...
    case "${action}" in
        enable)
            ...
            rm -rf /etc/ath/iface_mgr.conf /var/run/iface_mgr.pid
            iface_mgr_setup
        ;;
    esac
    lock -u /var/run/wifilock
